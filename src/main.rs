...
impl Blorb {
    ...
    pub fn has_chunk(&self, typeid: TypeID) -> bool {
        self.chunks
            .iter()
            .any(|chunk| chunk.typeid == typeid)
    }
...
fn main() -> anyhow::Result<()> {
    ...
    if blorb.has_chunk(TypeID(*b"ANNO")) {
        eprintln!("warning: Blorb already contains ANNO chunk, so no new one will be added");
    } else {
        let version = option_env!("GIT_HASH").unwrap_or("unknown");
        let date = jiff::Zoned::now()
            .in_tz("UTC")?
            .strftime("%Y-%m-%d %H:%M:%S")
            .to_string();
        let annotation = format!("Generated by Snavig ({version}) on {date}");
        blorb.add_chunk(Chunk::new(TypeID(*b"ANNO"), annotation.as_bytes().to_vec()));
    }
